...
mod test {
    ...
    fn test_hash2curve() {
        ...
        let pt1 = hash2curve(&AppId::from(app_id.to_vec())).unwrap();
        let pt2 = hash2curve(&AppId::from(app_id_same.to_vec())).unwrap();
        assert!(pt1 == pt2);

        let app_id = b"Hello Near!";
        let pt2 = hash2curve(&AppId::from(app_id.to_vec())).unwrap();
        assert!(pt1 != pt2);
    }
