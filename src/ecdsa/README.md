...
- **`RerandomizationArguments`** -- presignature rerandomization via HKDF-SHA3-256, used by both schemes to derive fresh randomness from contextual data (public key, tweak, message hash, participants, entropy)
- **`KeygenOutput`** / **`Tweak`** -- Secp256k1-specialized aliases for the curve-generic DKG output types
...
Originally imported from the [Cait-Sith](https://github.com/cronokirby/cait-sith) library. Uses an **offline phase with two protocols** (triple generation + presigning) to enable efficient one-round online signing.

See [`ot_based_ecdsa/README.md`](ot_based_ecdsa/README.md) for details.

### Robust ECDSA (`robust_ecdsa/`)

Implemented from scratch following \[[DJNPO20](https://eprint.iacr.org/2020/501)\]. Avoids Beaver triple generation entirely -- the offline round consists of a single presigning protocol using degree-2t polynomials. Requires `N >= 2t+1` signers where `t = MaxMalicious`.
