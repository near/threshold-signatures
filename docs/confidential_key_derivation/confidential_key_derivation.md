...
- $`\texttt{mvk}`$: master verification key of the *MPC network* corresponding to $\texttt{msk}$. It is a point in $\mathbb{G_2}$.
...
- When the *MPC Network* receives a new CKD request with parameters
  ...
  - Node $`i\in \{1, \ldots n\}`$ receives $`(\texttt{app\_id}, A)`$ and
    ...
    - $`S_i = x_i \cdot H(\texttt{\texttt{mvk}, app\_id})`$
    - $`C_i =  S_i + y_i \cdot A`$
  ...
  - The coordinator adds the received pairs together:
    ...
    \texttt{msk} \cdot H(\texttt{mvk},\, \texttt{app\_id}) + a \cdot Y`$
    - $`\texttt{es} \gets (Y, C) `$
...
- *app* obtains $`\texttt{es} = (Y, C)`$ and computes the BLS signature
  ...
  respect to the MPC network master verification key $`\texttt{mvk}`$. If correct, the app can
  use the computed $`\texttt{sig} = \texttt{msk} \cdot H(\texttt{mvk},\, \texttt{app\_id})`$ to
